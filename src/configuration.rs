...
pub struct EmailClientSettings {
    ...
    // New timeout configuration value!
...
impl DatabaseSettings {
    pub fn without_db(&self) -> PgConnectOptions {
        // SSL not relevant for local development - only for prod
        let ssl_mode = if self.require_ssl {
            PgSslMode::Require
        } else {
            // Try an encrypted connection, fallback to unencrypted it it fails
            PgSslMode::Prefer
        };
...
pub fn get_configuration() -> Result<Settings, config::ConfigError> {
    ...
    // Since docker doesn't see localhost as coming from the same machine we need different local and prod host address
    ...
    let settings = config::Config::builder()
        // Add configuration values from a file called either `base, local or production.yml'
        ...
        // Eg. `APP_APPLICATION_PORT=500` would set `Settings.application.port`
        .add_source(
